...
import mimetypes
import re
from munkiwebadmin.wrappers import wrap_basestring, readPlistFromString, writePlistToString
...
def normalize_value_for_filtering(value):
    ...
    if isinstance(value, (int, float, bool, wrap_basestring, dict)):
...
def convert_strings_to_dates(jdata):
    ...
    if isinstance(jdata, dict):
        for key, value in jdata.items():
            if ('date' in key.lower() and isinstance(value, wrap_basestring)
